...
if ((getPlayerUID player) call isAdmin) then
{
	...
	if (!_curPlayerInvulnState) then
	{
		...
		CCGLogger = ["AdminLog", format["Turned PlayerGod Mode on [%1 (%2)]", name player, getPlayerUID player]];
	...
	else
	{
		...
		CCGLogger = ["AdminLog", format["Turned PlayerGod Mode Off [%1 (%2)]", name player, getPlayerUID player]];
		publicVariableServer "CCGLogger";
	};
};
